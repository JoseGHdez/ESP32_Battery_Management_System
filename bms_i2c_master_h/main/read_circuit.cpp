...
#define I2C_MASTER_SDA_IO 32  // Pin SDA de tu puerto Grove
#endif
#ifndef I2C_MASTER_SCL_IO
#define I2C_MASTER_SCL_IO 33  // Pin SCL de tu puerto Grove
#endif

// Asignación de memoria real para los semáforos globales externos
SemaphoreHandle_t i2c_bus_mutex = nullptr;
SemaphoreHandle_t ads1115_chip_mutex = nullptr;
...
extern "C" void BatteryTask(void *pvParameters) {
  ...
  while (1) {
    ...
    vTaskDelay(pdMS_TO_TICKS(80)); // Small delay to ensure bus is free
    auto amp  = adc.ReadCurrent(0, 0.0f);
    vTaskDelay(pdMS_TO_TICKS(80)); // Small delay to ensure bus is free
...
i2c_master_bus_handle_t InitGroveI2C() {
    gpio_num_t sda_pin = (gpio_num_t)I2C_MASTER_SDA_IO; 
    gpio_num_t scl_pin = (gpio_num_t)I2C_MASTER_SCL_IO; 

    // [Tu excelente rutina de limpieza de bus por software se mantiene intacta]
    ...
    // =============================================================
    // NUEVA CONFIGURACIÓN CAMPO POR CAMPO PARA C++23
    // =============================================================
    ...
    bus_config.glitch_ignore_cnt = 7;  // <-- VITAL: Filtra picos de ruido electromagnético
    bus_config.intr_priority = 0;
    bus_config.trans_queue_depth = 5;
    bus_config.flags.enable_internal_pullup = true; // <-- Fuerza el estado alto
...
// NUEVA VERSIÓN DEL ESCÁNER DE BUS (Adaptada a ESP-IDF v5)
void ScanI2CBus(i2c_master_bus_handle_t bus_handle) {
    printf("--- Escaneando Bus I2C Maestro ---\n");
    int devices_found = 0;
    for (uint8_t addr = 1; addr < 127; addr++) {
        // i2c_master_probe verifica si un dispositivo responde al ACK en esa dirección
    ...
    if (devices_found == 0) printf("No se detectaron dispositivos I2C.\n");
    printf("----------------------------------\n");
}
...
extern "C" void app_main(void) {
  ...
  if (mountSPIFFS("/variables", "storage3", 8) == ESP_OK) {
    std::ifstream param_file("/variables/exp_vars.txt", std::ios::in);
    ...
    if (param_file.is_open()) {
      ...
      while (std::getline(param_file, line)) {
        ...
        if (std::getline(iss, key, '=') && iss >> value) {
          ESP_LOGI(TAG, "Loaded param: %s = %s", key.c_str(), value.c_str());
          if (key == "CHG_BIAS") {
            experiment_params.charge_bias = std::stof(value);
          } else if (key == "DIS_BIAS") {
            ...
            experiment_params.battery_capacity_mah = std::stof(value);
  ...
  // =============================================================
  // MODIFICACIÓN NUEVO DRIVER I2C
  // =============================================================
  // 1. Inicializamos el hardware y guardamos el manejador del bus único
  i2c_master_bus_handle_t i2c_bus_handle = InitGroveI2C();

  // 2. Escaneamos usando el nuevo método adaptado (pásale el handle)
  ScanI2CBus(i2c_bus_handle);

  // 3. Inicializamos los periféricos pasándoles el bus compartido
  static ServoController servo;
  static I2CRelay relay_controller(i2c_bus_handle); // <--- Ahora requiere el handle
  static ADS1115 ads_controller(i2c_bus_handle, ADS1115_ADDR); // <--- Ahora requiere el handle
  
  servo.init_servo();
  // ============================================================

  i2c_bus_mutex = xSemaphoreCreateMutex();
  ads1115_chip_mutex = xSemaphoreCreateMutex();
  ...
  xTaskCreate(BatteryTask, "BAT_MONITOR", 1024 * 4, &app_context, 5, NULL);
