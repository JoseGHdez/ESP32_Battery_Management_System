...
class ADS1115 {
 ...
 private:
  ...
  float point_zero_voltage_ = 2.54762f; // Voltage at 0A for current sensor calibration (this should be determined experimentally for your specific sensor)
};
...
float ADS1115::ReadVoltage(uint8_t channel, bool fz0430_flag) {
  ...
  uint8_t mux_bits = 0xC0 | (channel << 4); // MUX bits for single-ended input on the specified channel
  ...
  float voltage = raw * 0.00018754579f; // 0.1875
  //std::cout << "Debug ADS1115: Raw ADC value: " << raw << std::endl;
  //std::cout << "Debug ADS1115: Voltage read: " << voltage << " V" << std::endl;
...
float ADS1115::ReadCurrent(uint8_t channel) {
  ...
  ESPLOGV("ADS1115", "Voltage for current reading: %.3f V", voltage);
  
  // Example calibration for a current sensor with sensitivity of 0.033V/A
  // (2.53 when only ESP32 is used) (2.54762 when using additional components)
