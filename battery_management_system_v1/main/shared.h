...
#define GPIO_INPUT_IO_2     GPIO_NUM_35
...
struct ExperimentParams {
  ...
  float battery_capacity_ah; // Battery capacity in ampere-hours (used for calculating coulombs passed)
  ...
  std::string wifi_ssid; // WiFi SSID for connecting to the network (used for MQTT publishing)
  std::string wifi_password; // WiFi password for connecting to the network (used for MQTT publishing)
  std::string current_tag; // Custom tag for the current phase of the experiment (used in MQTT topic and logging)
  bool timer_flag; // Flag to indicate whether the timer has been enabled (used for safety cutoff based on time limits)
};
...
struct PhaseTaskParams {
  ...
  std::string     start_date;   // Date when the experiment started (for logging)
  float           mas_charge{0};     // Measured mAs charged at the end of the charging phase
  float           mas_discharge{0};  // Measured mAs discharged at the end of the discharging phase
  float           initial_voltage_cell_1{0}; // Initial voltage of cell 1 at the start of the phase
  float           initial_voltage_cell_2{0}; // Initial voltage of cell 2 at the start of the phase
  float           initial_voltage_cell_3{0}; // Initial voltage of cell 3 at the start of the phase
};
